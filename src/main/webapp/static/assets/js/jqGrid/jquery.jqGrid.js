...
$.extend($.jgrid,{
	...
	styleUI : {
		jQueryUI : {
			...
			base : {
				entrieBox : "ui-widget ui-widget-content ui-corner-all jqgrid-compact", // entrie div  incl everthing
				viewBox : "", // view diw
				headerTable : "",
...
$.jgrid.extend({
			...
			for(i = 0; i<clone; i++) {
				...
				if($t.p._nvtd) {
					if(o.dropmenu) {
                        ...
                        $($t).jqGrid('_buildNavMenu', pgid, elemids, p, pEdit, pAdd, pDel, pView );
	...
	//_buildNavMenu : function ( elem, elemids, p, pEdit, pAdd, pDel, pSearch, pView ) {
        _buildNavMenu : function ( elem, elemids, p, pEdit, pAdd, pDel, pView ) {
		return this.each(function() {
			var $t = this,
	...
	inlineNav : function (elem, o) {
		...
		return this.each(function(){
			...
			if(!$t.p.navGrid) {
                ...
                $($t).jqGrid('navGrid',elem, {refresh:false, edit: false, add: false, del: false,  view: false});
			}
			if(!$($t).data('inlineNav')) {
				$($t).data('inlineNav',o);
			}
