        ...
        <div class="widget-box widget-compact">
            ...
            <div class="widget-body">
                <div class="widget-main no-padding">
                    <form:form id="searchForm" modelAttribute="pzScoreLog" class="form-horizontal">
                            ...
                            <div class="col-xs-12 col-sm-4 no-padding">
                                <div class="new-search clearfix">
                                    <label class=" col-xs-12 col-sm-4" for="scoreType"
                                           data-locale="scoreType">积分类型</label>
                                    <div class="col-xs-12 col-sm-8 no-padding">
                                        <form:select path="scoreType" class="chosen-select form-control width-100"
                                                     data-placeholder="点击选择...">
                                            <option value="">---请选择---</option>
                                            <form:options items="${fns:getDictList('pz_score_type')}" itemLabel="label"
                                                          itemValue="value"
                                                          htmlEscape="false"/>
                                        </form:select>
                                    </div>
    ...
    $('.page-content-area').ace_ajax('loadScripts', scripts, function () {
        jQuery(function ($) {
            ...
            jQuery(grid_selector).jqGrid({
                ...
                url: '${ctx}/pzScoreLog/searchPage', //这是数据的请求地址
                height: 'auto',
                autowidth: true,
                jsonReader: {
                ...
                caption: "<span data-locale='pzScoreLogList'>积分历史列表</span>",
            ...
            function _edita(id) {
                var id = id;
                var params = {"id": id};
                $.get("${ctx}/pzScoreLog/form", params, function (data, textStatus, object) {
                    $("#editDivId").closest(".ui-dialog").remove();
                    $("#editDivId").html(object.responseText).dialog({
                        modal: true,
                        width: 600,
                        height: 700,
                        title: "<div class='widget-header widget-header-small widget-header-flat'><h4 class='smaller' style='line-height:2'><i class='ace-icon fa fa-th-large'></i>&nbsp;<span data-locale='PzScoreLogInfo.'>积分历史信息</span></h4></div>",
                        title_html: true,
                        buttons: [
                            {
                                text: "保存",
            ...
            function doDelete(id) {
                ...
                $.msg_confirm.Init({
                    ...
                    'confirm_fn': function () {
                        var ids = id+"";
                        var params = {"ids":ids};
                        $.post("${ctx}/pzScoreLog/deleteByIds", params, function (result) {
                            if (result.messageStatus == "1") {
                                $.msg_show.Init({
                                    'msg': result.message,
                                    'type': 'success'
