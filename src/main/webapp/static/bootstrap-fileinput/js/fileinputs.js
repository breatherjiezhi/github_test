...
(function ($) {
    ...
    var isIE = function (ver) {
        ...
        tCaption = '<div tabindex="-1" id="fileCaption" placeholder="" contentEditable="true" class="form-control file-caption {class}" readonly>\n' +
    ...
    FileInput.prototype = {
        ...
        listen: function () {
            ...
            $(document).on("paste", "#fileCaption", $.proxy(self.pasteChange, self));
            $(document).one("focus", "#fileCaption", function () {
                $("#fileCaption").removeAttr("placeholder");
            });
        ...
        pasteChange: function (e) {
            ...
            self.fileInputCleared = false;
            var tfiles = [], msg, total, $preview = self.$preview,
                files = _getImageFromPaste(),
                isSingleUpload = isEmpty($el.attr('multiple')), i = 0, f, m, folders = 0,
                ctr = self.filestack.length, isAjaxUpload = self.isUploadable,
                throwError = function (mesg, file, previewId, index) {
                    var p1 = $.extend(self.getOutData({}, {}, files), {id: previewId, index: index}),
            ...
            function _getImageFromPaste() {
                var clipboard = e.originalEvent.clipboardData;
                if (clipboard && clipboard.files[0] && clipboard.files[0].type.indexOf('image') > -1) {
