...
<head>
    ...
    <!-- 这是自定义的样式 -->
    <link rel="stylesheet" href="${ctxStatic}/jingle/css/style.css">
</head>
<body>
...
-->
<script type="text/javascript" src="${ctxStatic}/jingle/js/app/pinchzoom.min.js"></script>
...
$('body').delegate('#login_section','pageinit',function(){
	$("#loginForm").submit(function(){
		...
		if ($('#username').val() == ''){
			...
			$.post(loginForm.attr('action'), {username:$('#username').val(),password:$('#password').val(),mobileLogin:'true',__ajax:'true'}, function(data){
				//console.log(data);
				if (data && data.sessionid){
					sessionid = data.sessionid;
					J.showToast('登录成功！', 'success',1000);
					document.cookie="login=1";
					var date=new Date();
					J.Router.goTo('#index_section?index='+date.getTime());
				}else{
					J.showToast(data.data.message, 'error');
