...
$(function () {
    ...
    clickLogin();
    // 加载一段时间后自动登录
	//setTimeout(clickLogin, 400);
	// 调用预览功能
	setTimeout(clickStartRealPlay1, 1000);
...
//遍历所有的iChannelID 从2开始是为了解决第一个显示不出来，后面第一个独立拿出来
function clickStartRealPlay1() {
	
	for(var i=1;i<$('#channels option').length;i++){
		var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
		szIP = $("#ip").val(),
		iStreamType = 1,
		iChannelID = parseInt(i+1, 10),
		bZeroChannel = $("#channels option").eq($("#channels").get(0).selectedIndex).attr("bZero") == "true" ? true : false,
		szInfo = "";
		/*alert("szIP:  "+szIP)
        alert("iStreamType:  "+iStreamType)
        alert("iChannelID:  "+iChannelID)
        alert("bZeroChannel:  "+bZeroChannel)*/
		if ("" == szIP) {
		return;
		}
