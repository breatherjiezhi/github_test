...
public class ActModelService extends BaseService {
	 ...
	 * 流程模型列表
	 */
	public Page<ModelEntity> modelList(Page<ModelEntity> page, String category) {
		...
		List<Model> modelList =  modelQuery.listPage(page.getFirstResult(), page.getMaxResults());
		List<ModelEntity> list =  Lists.newArrayList();
		for (Model model : modelList) {
			ModelEntity modelEntity =  new ModelEntity();
			modelEntity.setCreateTime(model.getCreateTime());
			modelEntity.setId(model.getId());
			modelEntity.setKey(model.getKey());
			modelEntity.setLastUpdateTime(model.getLastUpdateTime());
			modelEntity.setName(model.getName());
			modelEntity.setVersion(model.getVersion());
