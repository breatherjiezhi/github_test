...
public class WxMenuController {
    ...
    @RequestMapping(value = {"findCurrentMenuByRid"}, method = RequestMethod.POST)
    @ResponseBody
    public Map<String, Object> findCurrentMenuByRid(String rid, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            ...
            //获取当前时间本周吃饭时间集合 用于查询本周订单
            List<String> currentWeekEatDate = TimeUtils.getCurrentWeekEatDate();
            //时间用逗号","进行拼接
            String eatDate = currentWeekEatDate.stream().collect(Collectors.joining(",")) + ",";
            ...
            //菜单接口需求id;
            int id = 1;
            for (String eDate : currentWeekEatDate) {
                Map<String, Object> map = new HashMap<>();
                map.put("id", "id" + id);
                map.put("eadDate", eDate);
                map.put("eadWeek", TimeUtils.getWeekDay(eDate));
    ...
    @RequestMapping(value = {"findCurrentRestaurant"}, method = RequestMethod.POST)
    @ResponseBody
    public Map<String, Object> findCurrentRestaurant(HttpServletRequest request, HttpServletResponse response, Model model) {
