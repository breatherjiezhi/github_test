package com.dhc.rad.modules.act.service;
...
public class ActProcessService extends BaseService {
	 ...
	 * 流程定义列表
	 */
	public Page<DeployProcess> processList(Page<DeployProcess> page, String category) {
	    ...
	    List<DeployProcess> list = Lists.newArrayList();
	    for (ProcessDefinition processDefinition : processDefinitionList) {
	      String deploymentId = processDefinition.getDeploymentId();
	      Deployment deployment = repositoryService.createDeploymentQuery().deploymentId(deploymentId).singleResult();
	      DeployProcess deployProcess = new DeployProcess();
	      deployProcess.setProcessId(processDefinition.getId());
	      deployProcess.setProcessKey(processDefinition.getKey());
	      deployProcess.setProcessName(processDefinition.getName());
	      deployProcess.setProcessDeploymentId(processDefinition.getDeploymentId());
	      deployProcess.setProcessDiagramResourceName(processDefinition.getDiagramResourceName());
	      deployProcess.setProcessResourceName(processDefinition.getResourceName());
	      deployProcess.setProcessSuspended(processDefinition.isSuspended());
	      deployProcess.setProcessVersion(processDefinition.getVersion());
	      deployProcess.setDeploymentDeploymentTime(deployment.getDeploymentTime());
	      deployProcess.setProcessCategory(processDefinition.getCategory());
	      list.add(deployProcess);
	    }
	    page.setList(list);
		return page;
	}
	 ...
	 * 流程定义列表
	 */
	public Page<ProcessInstanceEntity> runningList(Page<ProcessInstanceEntity> page, String procInsId, String procDefKey) {
	    ...
	    List<ProcessInstance>  processInstanceList =  processInstanceQuery.listPage(page.getFirstResult(), page.getMaxResults());
	    List<ProcessInstanceEntity>  list = Lists.newArrayList();
	    for (ProcessInstance processInstance : processInstanceList) {
	    	ProcessInstanceEntity processInstanceEntity = new ProcessInstanceEntity();
	    	processInstanceEntity.setActivityId(processInstance.getActivityId());
	    	processInstanceEntity.setBusinessKey(processInstance.getBusinessKey());
	    	processInstanceEntity.setId(processInstance.getId());
	    	processInstanceEntity.setParentId(processInstance.getParentId());
	    	processInstanceEntity.setProcessDefinitionId(processInstance.getProcessDefinitionId());
	    	processInstanceEntity.setProcessInstanceId(processInstance.getProcessInstanceId());
	    	processInstanceEntity.setTenantId(processInstance.getTenantId());
	        processInstanceEntity.setSuspended(processInstance.isSuspended());
