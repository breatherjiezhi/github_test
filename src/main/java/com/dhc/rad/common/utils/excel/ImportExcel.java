package com.dhc.rad.common.utils.excel;
...
import com.dhc.rad.common.utils.Reflections;
		...
		try{
			...
			if(row!=null)
			{
				...
				for(int numMR = 0; numMR < this.sheet.getNumMergedRegions(); numMR++){
	                ...
	                //如果当前单元格在这个合并单元格里
	                if(rowNum >= firstRowInd && rowNum <= lastRowInd && column >= firstColumnInd && column <= lastColumnInd){    
	                	rowNum = firstRowInd;
	                    column = firstColumnInd;
	                    break;
	                }  
				}
...
//					val = cell.getNumericCellValue();
					//为解决controller中 数量是null转double时的问题，此处初始化为0
...
//					val = cell.getCellFormula();//获取到的是公式
//					log.debug("getCellFormula() ------>    " + val);
...
//					log.debug("cellValue ------>    " + cellValue.getNumberValue());
			...
			for (Object[] os : annotationList){
				...
				if (val != null){
					...
					try {
						...
						System.out.println("Get cell value ["+i+","+column+"] error: " + ex.toString());
						val = null;
					}
					// set entity value
					if (os[1] instanceof Field){
